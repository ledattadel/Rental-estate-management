...
public class UserService implements IUserService {
    ...
    public List<StaffResponseDTO> getStaffs(long id) {
        ...
        List<UserEntity> staffs = userRepository.findByStatusAndRoles_Code(SystemConstant.ACTIVE_STATUS,
                "STAFF");
        BuildingEntity buildingFound = Optional.ofNullable(buildingRepository.findOne(id))
                .orElseThrow(() -> new NotFoundException("Customer not found!"));

        List<UserEntity> manageStaffs = buildingFound.getUsers();
        return staffs.stream().map(staff ->
                {
                    StaffResponseDTO staffRespone = userConverter.convertToResponse(staff);

                    for (UserEntity item : manageStaffs) {
                        if (staff.getId() == item.getId()) {
                            staffRespone.setChecked("checked");
                            break;
                        }
                    }
    ...
    @Override
    @Transactional
    public void assignmentBuilding(AssignmentBuildingRequest request) {

        BuildingEntity buildingEntity = Optional.ofNullable(buildingRepository.findById(request.getBuildingId()))
                .orElseThrow(() -> new NotFoundException("Building not found!"));
        List<Long> staffIds = request.getStaffIds();
        Long count = userRepository.countByIdIn(staffIds);

        if(count != staffIds.size()){
            throw new NotFoundException("Not found");
        } else {
            buildingEntity.setUsers(userRepository.findByIdIn(staffIds));
        }
        buildingRepository.save(buildingEntity);
    }

    @Override
    public Map<Long, String> findAllStaff() {
    ...
    @Override
    public List<StaffResponseDTO> getCustomerStaffs(long customerId) {
        ...
        List<UserEntity> manageStaffs = customerFound.getUsers();
        return staffs.stream().map(staff ->
                {
                    StaffResponseDTO staffRespone = userConverter.convertToResponse(staff);
    ...
    @Override
    public void assignmentCustomer(AssignmentCustomerRequest request) {
        CustomerEntity customerEntity = Optional.ofNullable(customerRepository.findOne(request.getCustomerId()))
                .orElseThrow(() -> new NotFoundException("Customer not found!"));
        List<Long> staffIds = request.getStaffIds();
        Long count = userRepository.countByIdIn(staffIds);
        ...
        if(count != staffIds.size()){
            ...
            customerEntity.setUsers(userRepository.findByIdIn(staffIds));
        }
